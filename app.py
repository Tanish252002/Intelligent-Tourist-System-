...
X = df[['City','Best_time_to_visit', 'Type', 'DistanceFromCity' ]]
y = df['Place']
...
merged_df = pd.merge(city_df, places_df, on="City")
...
df2 = pd.get_dummies(merged_df, columns=['Best_time_to_visit'])
df2['features'] = df2['City'] + ' ' + df2['Ratings_y'].astype(str) + ' ' + df2['DistanceFromCity'].astype(str)
...
def recommendtour(place, cosine_sim=cosine_sim):
    places = df2[df2['Place'].str.contains(place, case=False)]
    ...
    return df2['Place'].iloc[place_indices]
...
def recommend():
    if flask.request.method == 'GET':
        timetovisit      = request.args['timetovist']
        traveltype      = request.args['traveltype']
        distance        = request.args['distance']
        inpcity        = request.args['city']
        
        user_input = {'City':inpcity,'Best_time_to_visit': timetovisit, 'Type': traveltype, 'DistanceFromCity': distance}
        user_input_encoded = {col: label_encoders[col].transform([user_input[col]])[0] for col in user_input}
